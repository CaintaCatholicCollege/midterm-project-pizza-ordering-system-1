...
function finalTally(){
    ...
    totalT.innerHTML = "Total Amount: <span class='peso'>₱" +totalPrice+"<span>";
    }
    ...
    for (var p=0; p < paras.length-1; p++){
        paras[p].innerHTML = paraSplit[p][0]+" <span class='peso'>₱"+paraSplit[p][1]+"</span>";
...
//                console.log(CInputs()[j].parentElement);
              createPara('data-name',CInputs()[j].getAttribute('id'));
                 paras[paras.length - 1].innerHTML = ingredients+' <span>₱69</span>';
